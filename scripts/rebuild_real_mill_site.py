...
def organization_schema() -> dict:
    return {
        ...
        "image": [
            "https://urbanfresh.in/assets/images/ricefarm/mill-processing-plant.webp",
...
def page_hero(kicker: str, title: str, text: str, image: str, crumbs: list[tuple[str, str | None]]) -> str:
    crumb_html = "".join(
        f'<a href="{"/" if href == "index.html" else href}">{escape(label)}</a>'
        if href
...
def render_home() -> None:
    ...
    """)
    render_page("index.html", "Rice Mill in Karnal | UrbanFresh Rice Manufacturer", "UrbanFresh operates a fully automated rice production facility in Village Daha Madanpur, Karnal, producing basmati and non-basmati rice for Indian and export buyers.", body, "home", schema, image_path("mill-processing-plant.webp"), hero_image=image_path("mill-hero-1.webp"))
...
def render_infrastructure() -> None:
    ...
    body = page_hero("Rice processing infrastructure", "A mill built for controlled processing from paddy to pack.", "See the plant systems used for cleaning, parboiling, drying, milling, sorting and packaging at our Village Daha Madanpur manufacturing base.", "mill-infrastructure.webp", crumbs) + dedent(f"""
      <section class="section"><div class="container split"><div class="photo-frame"><img src="{image_path('mill-infrastructure.webp')}" alt="UrbanFresh rice mill infrastructure in Karnal" width="1000" height="760"></div><div><p class="section-label">Our plant</p><h2 class="section-title">A fully automated production facility.</h2><p class="section-lede">{AUTOMATION_CLAIM} Our daily production capacity is 230 metric tons, with connected equipment supporting preparation, processing, sorting and packing.</p><ul class="check-list"><li>Paddy procurement, drying and warehousing.</li><li>Cleaning, de-stoning, grading and separation.</li><li>Parboiling, drying, milling, sorting and polishing.</li><li>Packaging and logistics coordination.</li></ul></div></div></section>
...
def render_certifications() -> None:
    ...
    body = page_hero("Our registrations and certificates", "Documents available for buyer review.", "Review the certificate and registration images for our Village Daha Madanpur rice mill, then request current copies for your order.", "certificate-1.webp", crumbs) + dedent(f"""
      <section class="section"><div class="container"><div class="content-grid"><article class="prose"><h2>Our mill documents</h2><p>Our available records include ISO 22000:2018, FSSAI, APEDA, U.S. FDA registration, Importer Exporter Code, rice-mill registration and related compliance documents.</p><p>The mill is operated under the legal name {LEGAL_ENTITY}. UrbanFresh is our customer-facing brand.</p><div class="availability-note"><strong>Buyer verification:</strong> registrations can expire, renew or apply to a specific legal entity, unit, product or market. Request current full-resolution copies and verify validity and scope before placing an order.</div></article><aside class="info-panel"><h2>Need documents?</h2><p>Tell us the destination, rice, volume and certificates your buyer requires.</p><a class="button button-arrow" href="contact.html#quote">Request Current Copies</a></aside></div><div class="certificate-grid">{cards}</div></div></section>
