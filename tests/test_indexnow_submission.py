...
class IndexNowSubmissionTests(unittest.TestCase):
    ...
    def test_repository_sitemap_contains_only_the_configured_host(self) -> None:
        ...
        self.assertTrue(
            all(url.startswith("https://urbanfresh.in/") for url in urls)
        )
...
"""
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(RuntimeError, "urbanfresh.in"):
                submit_indexnow.sitemap_urls(path, "urbanfresh.in")
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"
  ...
  <url>
    <loc>https://urbanfresh.in/contact.html</loc>
    <image:image><image:loc>https://urbanfresh.in/assets/contact.webp</image:loc></image:image>
  </url>
</urlset>
"""
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "sitemap.xml"
            path.write_text(sitemap, encoding="utf-8")
            urls = submit_indexnow.sitemap_urls(path, "urbanfresh.in")
        self.assertEqual(urls, ["https://urbanfresh.in/contact.html"])
    ...
    def test_json_submission_uses_post_and_expected_payload(self) -> None:
        ...
        urls = ["https://urbanfresh.in/", "https://urbanfresh.in/contact.html"]
        with mock.patch.object(
